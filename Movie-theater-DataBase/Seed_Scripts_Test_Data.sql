...
-- Reset tables so seed script is repeatable
...
-- ===========================
-- We generate 80 showtimes using HelperNumber cross join
-- Different movies, auditoriums, dates, times, and formats.

INSERT INTO Showtime (movie_ID, auditorium_ID, format, language, showDateTime)
SELECT
  ((rn - 1) MOD 12) + 1 AS movie_ID,        -- cycle through 12 movies
  ((rn - 1) MOD 10) + 1 AS auditorium_ID,   -- cycle through 10 auditoriums
  CASE ((rn - 1) MOD 3)
    WHEN 0 THEN '2D'
    WHEN 1 THEN '3D'
    ELSE 'IMAX'
...
-- ===========================
-- 8. TICKETS (400+)
--    Sell seats for first 8 showtimes,
--    but not every seat, so some remain unsold.
-- ===========================
...
WHERE s.showtime_ID <= 8   -- only first 8 showtimes
ORDER BY s.showtime_ID, st.row_label, st.seatNumber
LIMIT 400;                 -- ensures at least 400 tickets, not full capacity